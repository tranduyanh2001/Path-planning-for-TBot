...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        else if (!is_safe_trajectory(trajectory, grid) || move_out == true)
        ...
        if (!trajectory.empty() && dist_euc(pos_rbt, pos_target) < close_enough || move_out == true)
        {
            if (--t < 0)
        ...
        if (replan)
        {
            if (grid.get_cell(pos_rbt) && grid.get_cell(pos_goal) || move_out == true)
            {
                ...
                { // path found
                    ...
                    {
                        Position &turn_pt_next = post_process_path[m - 1];  //position of next turn
                        Position &turn_pt_cur = post_process_path[m];       //position of current turn
                        ...
                        double f_angle = heading(turn_pt_cur, turn_pt_next); //heading of next point from this point
                        ...
                        double arbitrary_magnitude = sqrt(pow(Dx,2)+pow(Dy,2))/time_taken;
                        
                        //double arbitrary_magnitude = 0.05;
                        //NO change here
                        double f_vel_x = arbitrary_magnitude * cos(f_angle); //vector decoupling
                        double f_vel_y = arbitrary_magnitude * sin(f_angle); //vector decoupling
                        ...
                        std::vector<Position> traj = generate_trajectory(turn_pt_next, turn_pt_cur, average_speed, target_dt, grid, i_vel_x, i_vel_y, f_vel_x, f_vel_y);
            ...
            { // robot lies on inaccessible cell, or if goal lies on inaccessible cell
                if (!grid.get_cell(pos_rbt)){
                    ...
                    // prev_pos_rbt = pos_rbt;

                    // while (true) {

                    //     pub_target.publish(msg_target);
                    //     msg_target.point.x = nearest_safe.x;
                    //     msg_target.point.y = nearest_safe.y;

                    //     if (pos_rbt.x == nearest_safe.x && pos_rbt.y == nearest_safe.y ){
                    //         move_out = true;
                    //         break;
                    //     }
