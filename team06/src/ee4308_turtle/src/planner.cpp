    ...
    while (!open_list.empty())
    {
        ...
        {   // for each neighbor in the 8 directions
            ...
            {   // previous cost was more expensive, rewrite with current
                ...
                // add to open
                add_to_open(&nb_node,false); // & a reference means getting the pointer (address) to the reference's object.
            }
...
Position Planner::dijkstra(Position rbt_position){
    ...
    for(Index & idx : path_idx_dijkstra){
        if(grid.get_cell(grid.idx2pos(idx))){   //if grid is accessible
            safe_pt = grid.idx2pos(idx);        //assign nearest available point on grid
            break;
        }
    }
...
std::vector<Index> Planner::dijkstra(Index idx_current){
    ...
    for (Node & node : nodes)
    {
        node.g = 1e7; // a reasonably large number. You can use infinity in clims as well, but clims is not included
        node.visited = false;
    }
    ...
    while (!open_list.empty())
    {
        ...
        {   // for each neighbor in the 8 directions
            ...
            // set is_closest_point true if neighbors are accessible and direction is at final itiration
            ...
            // {   // previous cost was more expensive, rewrite with current
                ...
                // add to open
                add_to_open(&nb_node,true); // & a reference means getting the pointer (address) to the reference's object.
            // }
